var TextSpinner = function(parent, ledger, subArr, names, font, hitBoxes) {
...
*/
		//this.addSubObjects(names.length, this.boxSize+16);
		this.addSubObjects(names.length, 30);
			...
			//	this.subArr.push(sphere)
				...
				var msg = names[j];
				 ...
				 var bmtext = new TextBitmap({
			          imagePath: 'fonts/Lato-regular.png',
			          text: msg,
			          width: 600,
			          align: 'center',
			          font: this.font,
				 ...
				 bmtext.group.position.x = 0
				 bmtext.group.position.y = r * Math.cos(theta);
				 bmtext.group.position.z = r * Math.sin(theta);

				 bmtext.group.rotation.x = Math.atan2( -bmtext.group.position.z, -bmtext.group.position.y )+ Math.PI / 2;;

        //orbitControls.target.copy( bmtext.group.position );

		        this.add( bmtext.group );
		        bmtext.group.parentSpinner = this;
...
*/
				...
				sphere.position.x = 0;//r * Math.cos(theta- (Math.PI/2)) ;
			//	sphere.xpos =0
...
TextSpinner.prototype = Object.create(THREE.Object3D.prototype);
TextSpinner.prototype.constructor = TextSpinner;

TextSpinner.prototype.setDeltaX = function(num) {
	console.log('********* Delta X = '+num);
    this.deltaX = num;
}

TextSpinner.prototype.setDeltaY = function(num) {
    this.deltaY = num;
}



TextSpinner.prototype.expand = function() {
...
TextSpinner.prototype.hideAll = function() {
		for ( var j = 0; j < this.subArr.length; j ++ ) {
...
TextSpinner.prototype.showAll = function() {
		for ( var j = 0; j < this.subArr.length; j ++ ) {
	    	TweenMax.to(this.subArr[j].position, 1, {
	        	x: this.subArr[j].xpos,
	        	y: this.subArr[j].ypos,
...
/*
TextSpinner.prototype.getMesh = function() {
