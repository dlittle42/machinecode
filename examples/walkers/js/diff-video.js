var DiffCamEngine = (function() {
	...
	function init(options) {
		...
		snapshotCanvas = options.snapshotCanvas || document.createElement('canvas');
		...
		snapshotCallback = options.snapshotCallback || function() {};
	...
	//	motionContext.globalCompositeOperation = 'source-over';


		snapshotCanvas.width = diffWidth;
		snapshotCanvas.height = diffHeight;
	...
	function takeSnapshot(){
		...
		var avgArr=[];
			...
			for (var r=0; r<hCount; r+=1) {
			    for (var c=0; c<vCount; c+=1) {
			    	var xpos = (w * r);//*spacing;
			    	console.log(xpos)
			    	var ypos = (h *c);//*spacing;
			        var diffImageData = motionContext.getImageData(xpos, ypos, w, h);
			        var glev = getAvg(diffImageData.data);
			        console.log(glev)
			        avgArr.push(glev);

					snapshotContext.putImageData(diffImageData, xpos, ypos);
			    }
			}

/*
		var diffImageData = motionContext.getImageData(0, 0, diffWidth, diffHeight);
		//var diff = processDiff(diffImageData);
		snapshotContext.putImageData(diffImageData, 0, 0);
*/
		//var diffImageData = motionContext.getImageData(0, 0, diffWidth/2, diffHeight/2);
		//var diff = processDiff(diffImageData);
		//snapshotContext.putImageData(diffImageData, 0, 5+ diffHeight/2);
		console.log(avgArr)
		snapshotCallback({avgArr});
	}
	...
	function adjustPixels(val){
		diffWidth= val;
		diffHeight = val;
		diffCanvas.width = val;
		diffCanvas.height = val;

		motionCanvas.width = val;
		motionCanvas.height = val;

		snapshotCanvas.width = val;
		snapshotCanvas.height = val;
